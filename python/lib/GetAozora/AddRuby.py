...
class AddRuby:
    ...
    def __post(self, query: str):
        ...
        param_dic = {
            ...
            "params": {"q": query, "grade": self.__grand},
        }
    ...
    def add_HTML_ruby(self, text: str):
        response = self.__post(text)
        t = ""
                ...
                if "subword" in word:  # カナ交じりだったらsubwordが含まれる
                    for kanamajiri in word["subword"]:
                        if p.fullmatch(kanamajiri["surface"]):
                            t += Template(
                                "<ruby><rb>$surface</rb><rt>$furigana</rt></ruby>"
                            ).substitute(
                                surface=kanamajiri["surface"],
                                furigana=kanamajiri["furigana"],
                            )
                        else:
                    ...
                    t += Template(
                        ...
                        surface=word["surface"], furigana=word["furigana"]
                    )  # 漢字のみのフリガナ設定
            else:
                t += str(word["surface"])
        return t
